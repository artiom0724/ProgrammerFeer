...
#define CURRENT_TOTAL_TIME 26000
int loop_time = 0;
...
long timing = 0, file_time = 0;
long currentFilePosition = 0;
bool isNextFigure = false;
bool isreturn;
...
void loop() {
  ...
  switch (currentEffect) {
    case CURRENT_MODE: current_mode(); break;
    case FILE_MODE: file_mode(); break;
    case BLUETOOTH_MODE: bluetooth_mode(); break;
...
{
  loop_time++;
  if (loop_time > CURRENT_TOTAL_TIME)
  {
    clearCube();
    loading = true;
    timer = 0;
...
void file_mode()
{
  ...
  if(timing < file_time){
    ...
    if(myFile){
      while (fromFile != '#' && myFile.available())
      {
        fromFile = myFile.read();
        Serial.print(fromFile);
        if (fromFile == 't')
        {
          isTemplate = myFile.read();
          isreturn = file_worker();
          renderCube();
          return;
          }
...
bool file_worker()
{
  while (fromFile != '#'&& fromFile != '\n' && myFile.available())
          {
            if(!isNextFigure)
            {
              if(fromFile=='#'|| fromFile == '\n'|| !myFile.available()|| fromFile == EOF)
                {
                      myFile.seek(0);
                      isreturn = false;
                      fromFile = '*';
...
int bluetooth_time = 0;
long blue_timing = 0, blue_time = 0;
bool isVoxel = true;
void bluetooth_mode()
{
  blue_time++;
  
  if(blue_time > blue_timing)
  {
    blue_time = 0;
    char value;
    clearCube();
    if (Serial.available()>0)
    {
      ...
      {
        if (Serial.available()<0 || value == '#')
        {
          return;
        }
        value = (char)Serial.read();
      }
      while (Serial.available()>0 && value!='\n' && value != '#')
      {
      for (int i = 0; i < 8; i++)
        for (int j = 0; j < 8; j++)
          for (int k = 0; k < 8; k++)
...
void text(const char string[], uint8_t len) {
  if (loading) {
    clearCube();
    charPosition = -1;
    charCounter = 0;
  ...
  if (timer > TEXT_TIME) {
    ...
    if (charPosition == 0) {
      for (uint8_t i = 0; i < 8; i++) {
        cube[i][0] = characters[string[charCounter] - '0'][7-i];
      }
    }
  }
}
